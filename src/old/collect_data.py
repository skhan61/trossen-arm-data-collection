...
def main():
    ...
    logger.info("")
    logger.info(
        f"Chessboard: {BOARD_SIZE[0]}x{BOARD_SIZE[1]} internal corners, {SQUARE_SIZE*1000}mm squares"
    )
    logger.info("=" * 60)
    logger.info("")
    ...
    logger.info(
        f"  Joint positions: {[f'{np.degrees(j):.2f}°' for j in initial_joint_positions]}"
    )
    logger.info(
        f"  Cartesian: x={initial_cartesian_pos[0]:.3f}, y={initial_cartesian_pos[1]:.3f}, z={initial_cartesian_pos[2]:.3f}"
    )
    logger.info("")
    ...
    all_data = {"initial_robot_state": initial_state, "poses": [], "num_poses": 0}
    ...
    try:
        while True:
            ...
            # Detect chessboard
            detected, corners, rvec, tvec = detect_chessboard(
            ...
            if detected:
                ...
                cv2.putText(
                    display,
                    "BOARD DETECTED & LOCKED - Press 's' to save and quit",
                    (10, 30),
                ...
                cv2.putText(
                    display,
                    "No board - Move robot to see chessboard",
                    (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.6,
                    (0, 0, 255),
