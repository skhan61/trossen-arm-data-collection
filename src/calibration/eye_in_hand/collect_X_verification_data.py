...
5. Press 'q' to quit
6. All data saved to Xverification/

Usage:
    python src/calibration/eye_in_hand/collect_X_verification_data.py
"""
...
# Directories
BASE_DIR = Path(__file__).parent  # src/calibration/eye_in_hand/
DATA_DIR = BASE_DIR / "Xverification"
...
def get_realsense_intrinsics():
    ...
    logger.info("Camera intrinsics from RealSense:")
...
def main():
    ...
    # X is not needed for data collection - verify_X.py loads X separately
    logger.info("Note: X will be loaded separately by verify_X.py")
    logger.info("")
    ...
    try:
        while True:
            ...
            if key == ord("s") and detected:
                ...
                logger.info(
                    "Enabling gravity compensation - move robot by hand for next pose"
                )
                driver.set_all_modes(Mode.effort)
                driver.set_all_efforts([0.0] * num_joints)
                time.sleep(0.5)
