...
LOG_FILE = (
    LOG_DIR / f"gelsight_calibration_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
)
...
class DataCollector:
    ...
    def __init__(self, openings: list, repeats: int):
    ...
    def connect_camera(self):
        ...
        logger.info(
            f"Camera connected! fx={self.intrinsics.fx:.2f}, fy={self.intrinsics.fy:.2f}"
        )
    ...
    def run(self):
        ...
        logger.info(
            f"Openings: {len(self.openings)} | Repeats: {self.repeats} | Total: {total_samples}"
        )
        logger.info("=" * 70)
        ...
        logger.info(
            f"Robot pose: [{cartesian[0] * 1000:.1f}, {cartesian[1] * 1000:.1f}, {cartesian[2] * 1000:.1f}] mm"
        )
        ...
        try:
            for opening_idx, opening in enumerate(self.openings):
                logger.info(
                    f"=== Opening {opening_idx + 1}/{len(self.openings)}: {opening * 1000:.1f}mm ==="
                )
                ...
                for rep in range(self.repeats):
                    ...
                    while time.time() - start_time < PREVIEW_TIME:
                        ...
                        if color_image is not None:
                            ...
                            cv2.putText(
                                display,
                                f"Opening {opening_idx + 1}/{len(self.openings)}: {actual_opening * 1000:.1f}mm | Rep {rep + 1}/{self.repeats}",
                                (10, 30),
                            ...
                            cv2.putText(
                                display,
                                f"Sample {sample_idx}/{total_samples} | q to abort",
                                (10, 60),
                                cv2.FONT_HERSHEY_SIMPLEX,
                                0.5,
                                (0, 255, 255),
                    ...
                    if color_image is not None and depth_image is not None:
                        ...
                        rgb_file = DATA_DIR / f"{img_prefix}_rgb.png"
                        depth_file = DATA_DIR / f"{img_prefix}_depth.npy"
            ...
            output_file = DATA_DIR / "calibration_data.json"
            with open(output_file, "w") as f:
                json.dump(output, f, indent=2)
            logger.info(f"Saved {len(all_data)} samples to {DATA_DIR}")

    def go_home(self):
        """Return gripper to home (closed) position.

        Note: Not called by default - gripper stays in current position.
        """
...
def collect(
    ...
    """
    openings = np.linspace(
        gripper_max / 1000, gripper_min / 1000, num_openings
    ).tolist()

    logger.info("Gelsight Calibration Data Collection")
    ...
    collector = DataCollector(openings=openings, repeats=repeats)
